    ...
    def experiment(self):
        ...
        for k in range(self.ntimes):
            ...
            for i in range(self.kfold):
                ...
                y_train, y_valid = self.split_ydata(y_labels, train_idx, valid_idx)

                x_train_fb = X_cv_extracted[train_idx]
                x_valid_fb = X_cv_extracted[valid_idx]

                train_loss = LAC.fit(classifier, x_train_fb, y_train, LAC.CrossEntropyWithLogPrior)
        ...
        det_test_predictions = DC.predict(det_classifier, X_test_extracted)
        ...
        fig = plt.figure()
        loss_curves = fig.add_subplot(121, title='Loss Curves')
        ...
        valid_acc = fig.add_subplot(122, title='Validation accuracy')
        ...
        fig1 = plt.figure()
        Shannon_H = fig1.add_subplot(111, title='Entropies')
        Shannon_H.plot(ratio, entropies, 'b-', label='laplace')
        Shannon_H.plot(ratio, det_entropies, 'g--', label='determ')
        Shannon_H.legend()
        fig1.savefig('entropies.jpg')
