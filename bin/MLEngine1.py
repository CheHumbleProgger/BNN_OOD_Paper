# This is former file of MLEngine which was used
import numpy as np
...
from bin.BNNClassifier import BayesianLayer
from bin.DeterministicClassifier import DeterministicClassifier
...
class MLEngine:
    ...
    def experiment(self):
        ...
        #print(type(y_labels), type(filtered_data), type(filtered_data[0])) all are numpy.ndarray
        #print(filtered_data.shape) #(9, 288, 22, 501), y_labels.shape = (288,)
        #print(y_labels)
        #y_range = range(len(y_labels)+1)
        #for y_local in y_range:
        #    if y_labels[y_local] == 2 or y_labels[y_local] == 3: #axis??
        #        y_labels = np.delete(y_labels, y_local)
        #        filtered_data = np.delete(filtered_data, y_local, axis=1) #index out of range error
        #        print(y_labels[y_local])
        #    print(y_local, y_labels.shape, filtered_data.shape)
        #print(y_labels)

        # TODO split data into test and CV datasets
        # X_train, X_test, y_train, y_test = train_test_split(filtered_data, y_labels, test_size=0.1)
        # Preprocessor starts FOR CV DATA ONLY!!!
        y_local = 0
        ...
        classifier_type = LAC.LaplaceApproximationClassifier()
        ...
        for k in range(self.ntimes):
            ...
            for i in range(self.kfold):
                ...
                #fbcsp = FBCSP(self.m_filters) # initialize outside training loop??
                ...
                for j in range(n_classes): # for class in all classes
                    ...
                    x_features_train = fbcsp.transform(x_train_fb, class_idx=cls_of_interest) # Why it needs class of interest??
                    x_features_test = fbcsp.transform(x_test_fb, class_idx=cls_of_interest)
                    # classifier_type = SVR(gamma='auto')
                    # classifier_type = LAC.LaplaceApproximationClassifier()  # FIXME problem with model initialization, it initializes every fold of every epoch.
                    # classifier = Classifier(classifier_type)
                    train_loss, y_train_predicted[:, j] = classifier.fit(x_features_train, np.asarray(y_train_cls, dtype=np.float)) # here model trains only on one class on every iteration
                ...
                # TODO Implement ROC-AUC metric usage
        ...
        # TODO implement predictions on test set

        # x_features = fbcsp.transform(X_test)
        # test_predictions = classifier.predict(X_test)

        # TODO implement some metrics for obtaining accuracy on predictions

        fig = plt.figure()
        ax0 = fig.add_subplot(121, title='train_loss')
        ax0.plot(epochs, losses, 'bo-', label='train')
        ...
        # TODO implement predictions on test set ( in which cycle I should do it?)
