...
class MLEngine:
    ...
    def experiment(self):
        ...
        ood_bcic_data = LoadData.LoadBCIC('A07T.gdf', self.data_path)
        ...
        ind_variances.errorbar([n for n in range(len(new_test_predictions))], new_test_predictions, yerr=ind_sd, fmt='o')
        ood_variances = fig4.add_subplot(122,title='ood_preds', xlabel='sample', ylabel='pred+-sd')
        ood_variances.errorbar([n for n in range(len(ood_new_test_predictions))], ood_new_test_predictions, yerr=ood_sd, fmt='o')
        fig4.savefig('ind_ood_2subj_variances.jpg')
